...
(rf/reg-event-fx
  :search-changed
  (fn [{db :db} [_ new-search]]
    (let [old-search (:location/search db)
          get-event  (:context/routes db)]
      (when-not (= old-search new-search)
        {:db         (assoc db :location/search new-search)
         :dispatch-n (let [old-context (:context old-search)
                           new-context (:context new-search)]
                       (if (not= old-context new-context)
                         (cond-> []
                                 (some? (get-event old-context)) (conj [(get-event old-context) :deinit new-search old-search])
                                 (some? (get-event new-context)) (conj [(get-event new-context) :init new-search]))
                         (cond-> []
                                 (some? (get-event old-context)) (conj [(get-event old-context) :params new-search old-search]))))}))))
     ...
     (if-let [route (route-map/match [:. path] (:route-map/routes db))]
       (let [params (assoc (:params route) :params q-params)
         ...
         {:db (assoc db
          ...
          :dispatch-n (conj (into context-evs page-ctx-events) [qs/on-hash-change (:q q-params)])})
       {:db (assoc db
...
(rf/reg-event-fx
 :route-map/init
  (fn [cofx [_ routes context-routes]]
    {:db             (assoc (:db cofx)
